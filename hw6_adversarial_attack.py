...
class Attacker:
    ...
    def attack(self, epsilon):
        ...
        for (data, target) in self.loader:
            ...
            image = perturbed_data * torch.tensor(self.std, device = device).view(3, 1, 1) + torch.tensor(self.mean, device = device).view(3, 1, 1)
...
if __name__ == '__main__':
    # 讀入圖片相對應的 label
    df = pd.read_csv(img_dir+"/labels.csv")
    df = df.loc[:, 'TrueLabel'].to_numpy()
    label_name = pd.read_csv(img_dir+"/categories.csv")
    label_name = label_name.loc[:, 'CategoryName'].to_numpy()
    # new 一個 Attacker class
    attacker = Attacker(img_dir+'/images', df)
    # 要嘗試的 epsilon
    epsilons = [0.02]
